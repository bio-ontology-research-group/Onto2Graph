...
(function($, lodLiveProfile) {
	...
	var methods = {
			...
			$.each(lodLiveProfile.connection, function(key, value) {
				...
				if(entry.length>0){
					...
					if(graphName==resource){
						...
						res = res.replace(/\{ROOT\}/ig, entry[0].root);
					}else{
						res = res.replace(/\{URI\}/ig, graphName.replace(/^.*~~/, ''));
						res = res.replace(/\{ROOT\}/ig, resource);
					}
					if (value.proxy) {
		...
		format : function(destBox, values, uris, inverses) {
			...
			if(thisUri==graphName){
				result+=$.trim(unescape(titles[0])) + " \n";
			}else{
				result+=thisUri;
			}
			...
			if (inverses) {
				...
				$.each(inverses, function(key, value) {
					for (var akey in value) {
						//To control the root is not repeated. 
						if(value[akey]=="owl%3AThing"){
							continue
						}
				...
				$.jsonp({
					...
					success : function(json) {
						...
						$.each(json, function(key, value) {
							...
							if (value.object.type == 'uri' || value.object.type == 'bnode' || value.object.type == 'literal'){
								...
								if (value.object.value != anUri) {
									if (value.object.type == 'bnode') {
										...
										//eval('uris.push({\'' + value['property']['value'] + '\':\'' + escape(value.object.value) + '\'})');
										eval('uris.push({\'' + value['relation']['value'] + '\':\'' + escape(value.object.value) + '\'})');
									}
								}
							} else {
								eval('values.push({\'' + value['property']['value'] + '\':\'' + escape(value.object.value) + '\'})');
