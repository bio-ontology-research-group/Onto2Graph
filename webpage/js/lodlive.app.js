$(function() {
	...
	if (loca) {
		...
		$('#firstLineBoxes').width((pag) * 930);
		$('#nextPage,#prevPage').click(function() {
			var boxes = $('#firstLineBoxes').not(':animated');
			...
			if (boxes.length == 1) {
				...
				if (next) {
					...
					} else if ($('#firstLineBoxes').css('marginLeft').replace(/[-px]/g, '') == (pag - 2) * 930) {
		...
		$.each($.jStorage.get('profile').connection, function(key, value) {
			...
			$('#startPanel').children('#boxes').children('#firstLineBoxes').append(aBox);
			...
			$('#startPanel').children('#boxes').children('#firstLineBoxes').append(descrBox);
			...
			var invia = $('<div class="inviaForm"> Start '+key+'>></div>');
