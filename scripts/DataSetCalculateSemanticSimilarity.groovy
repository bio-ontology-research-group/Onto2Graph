 ...
 * and BioGRID annotations across all organisms (fly, worm, fish, yeast and mouse)
 * taking into account Genetic and Physical interaction.
...
def parseFiles = {String dataSourceType, String interactionType->
    if(dataSourceType!=null){
        ...
        switch (dataSourceType) {
            case "GA"://Gene Association
                def geneInteractions = [];
                geneInteractions.add("resources/gene_association/fish/gene_association.zfin");
                geneInteractions.add("resources/gene_association/fly/gene_association.fb.gz.txt");
                geneInteractions.add("resources/gene_association/mouse/gene_association.mgi");
                geneInteractions.add("resources/gene_association/worm/gene_association.wb");
                geneInteractions.add("resources/gene_association/yeast/gene_association.sgd");
                geneInteractions.each { file ->
                ...
                annotationsList.each { file ->
                    ...
                    parseAnnotationFile(fAnnotations, 1);
                }
                return;
            case "BG"://BioGRID
                def bgInteractions = [];
...
if((fOnto!=null)&&(dataSourceType!=null)&&(interactionType!=null)&&(sSmConf!=null)){

    //0. Parse the annotation and the interaction file.
    parseFiles(dataSourceType,interactionType);
